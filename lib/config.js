...
module.exports = {
    load: function (config_path) {
        ...
        config.include_changelog = Boolean(config.include_changelog);
        config.include_aging_wip = Boolean(config.include_aging_wip);
        config.changelog_concurrency = parseInt(config.changelog_concurrency, 10) || 5;
        config.issue_types = Array.isArray(config.issue_types) ? config.issue_types : null;
        config.tls_reject_unauthorized = config.tls_reject_unauthorized !== false;
