...
function write_charts(dir_path, issues, working_times, config) {
    ensure_dir(dir_path);
    var lines = build_lines(issues, working_times, config);
    var written = chart_builder.build(lines, config).map(function (chart) {
    ...
    run: function (options) {
        ...
        return extract.run(config).then(function (result) {
            if (config.charts_dir && result.issues) {
                write_charts(config.charts_dir, result.issues, result.working_times, config);
            }
