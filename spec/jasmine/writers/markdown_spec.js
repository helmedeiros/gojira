...
describe('writers/markdown', function () {
    ...
    it('starts with a header row and a separator row', function () {
        var out = markdown_writer.build(issues, working_times, config);
        var rows = out.split('\n');
        expect(rows[0]).toBe('| Type | Key | Summary | Status | Points | Projected | Actual | Over/Under | Ratio |');
        expect(rows[1]).toBe('|------|-----|---------|--------|--------|-----------|--------|------------|-------|');
    });
    ...
    it('emits one row per issue', function () {
        ...
        expect(rows[2]).toContain('DEMO-1');
        expect(rows[2]).toContain('Sample');
    });
});
